...
function App() {
  ...
  return (
    <div className="min-h-screen bg-ink-950 text-ink-100">
      ...
      {view.name === 'dashboard' && (
        <Dashboard servers={servers} onServersChange={handleServersChange} onNavigate={handleNavigate} />
      )}
      {view.name === 'create' && (
        <CreateServerForm onCreated={(id) => handleNavigate({ name: 'details', serverId: id })} onBack={() => handleNavigate({ name: 'dashboard' })} />
      )}
      {view.name === 'details' && (
        <ServerDetails serverId={view.serverId} onBack={() => handleNavigate({ name: 'dashboard' })} />
      )}
    </div>
  );
}
